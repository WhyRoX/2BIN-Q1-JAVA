...
public class EmployeeManagement {
    ...
    private static final Supplier<Stream<String>> supplier = () -> {
        ...
        // créer un stream a partir de fichier streamingvf.csv
        try {
            return Files.readAllLines(Paths.get("Atelier 4 - 10-10/resources/streamingvf.csv")).stream();
        } catch (IOException e) {
            throw new UncheckedIOException(e);
        }
    };

    private static final Supplier<Stream<Employee>> employee = () -> supplier.get().map(Employee::new);
    ...
    private static List<String> filteredLastnames() {
        ...
        // split the string by the point-virgule
        /*return supplier.get()
                .map(s -> s.split(";") [1])
                .filter(predicate)
         ...
         */
        return employee.get().map(Employee::getLastname)
                .filter(predicate)
                .filter(s -> s.contains("O") || s.contains("K"))
                .toList();
    }
    ...
    private static List<Integer> occurencesOfE() {
        ...
        BiFunction <String, Character, Integer> occurences  = (s, c) -> {
                int count = 0;
    ...
    private static long numbreOfPartTimers() {
        ...
        return employee.get()
                .filter(employee -> !employee.isFullTime())
                .count();
    }

    /**
     * Catégorise les employés selon qu'ils travaillent à mi-temps(MT) ou à plein temps(TP).
    ...
    private static Map<Boolean, List<Integer>> timeDistrubution() {
        /*return supplier.get()
                .collect(Collectors.partitioningBy(s -> s.split(";")[4].equals("TP"),
                        Collectors.mapping(s -> Integer.parseInt(s.split(";")[0]), Collectors.toList())));
